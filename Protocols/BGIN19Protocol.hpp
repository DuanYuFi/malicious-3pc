...
BGIN19Protocol<T>::BGIN19Protocol(Player &P) : ReplicatedBase(P)
{
    ...
    for (int i = 0; i < global_thread_number; i ++) {
        threads.push_back(make_shared<thread>(&BGIN19Protocol<T>::thread_handler, this));
    }
#endif

    eval_p_poly = new gfp0[2 * global_k_size - 1];
    eval_base = new gfp0[global_k_size];
    eval_result = new gfp0*[global_k_size];
    for(int i = 0; i < global_k_size; i++) {
        eval_result[i] = new gfp0[global_k_size];
    }

    mul_triples = new MultiShare[global_batch_size * global_max_status * 2];
    thetas = new gfp0*[global_max_status];
    betas = new gfp0*[global_max_status];
    ...
    for (int i = 0; i < global_max_status; i ++) {
        thetas[i] = new gfp0[s];
        betas[i] = new gfp0[global_k_size];
    }

    int cnt = log(2 * _T) / log(global_k_size) + 1;
    rs_as_prover = new gfp0*[global_max_status];
    rs_as_verifier1 = new gfp0*[global_max_status];
    rs_as_verifier2 = new gfp0*[global_max_status];

    for (int i = 0; i < global_max_status; i ++) {
        rs_as_prover[i] = new gfp0[cnt + 1];
        rs_as_verifier1[i] = new gfp0[cnt + 1];
        rs_as_verifier2[i] = new gfp0[cnt + 1];
    }
...
void BGIN19Protocol<_T>::_PROVE1(int tid) {
    ...
    for (int i = 0; i < k; i++) {
        ...
        for (int j = 0; j < cols; j++) {
            if (temp_pointer >= size) {
                ...
                gfp0 x_first = mul_triples[global_batch_size * tid + temp_pointer].x[0];
                gfp0 y_first = mul_triples[global_batch_size * tid + temp_pointer].y[0];
                gfp0 x_second = mul_triples[global_batch_size * tid + temp_pointer].x[1];
                gfp0 y_second = mul_triples[global_batch_size * tid + temp_pointer].y[1];
                gfp0 rho_first = mul_triples[global_batch_size * tid + temp_pointer].rho[0];
                gfp0 rho_second = mul_triples[global_batch_size * tid + temp_pointer].rho[1];
                gfp0 z_second = mul_triples[global_batch_size * tid + temp_pointer].z[1];
    ...
    Langrange<gfp0>::get_bases(k, data.base);
    data.s = s * 2;
...
void BGIN19Protocol<_T>::_PROVE2(int tid) {
    ...
    Langrange<gfp0>::evaluate_bases(k, rs_as_prover[tid][verify_round], eval_base);
...
void BGIN19Protocol<_T>::prove(int size) {
    ...
    for (int i = 0; i < total; i ++) {
        factory.push(ThreadInfo(PROVE1, i));
    }
    ...
    while(true){
        ...
        for (int i = 0; i < total; i ++) {
            ...
            zkp_datas[i].verify_proof.p_evals_masked.push_back(vector<gfp0>());
            os[0].store(ss.size());
            gfp0 rand;
            for (auto &s: ss) {
        ...
        ws.set_target(total);
        for (int i = 0; i < total; i ++) {
            factory.push(ThreadInfo(PROVE2, i));
        }
        ws.wait();
...
void BGIN19Protocol<_T>::_VERIFY1(int tid) {
    ...
    vector<gfp0> b_ss(len);
    gfp0 final_input, final_result_ss;
    ...
    gfp0* eval_base = new gfp0[k];
    gfp0* eval_base_2k = new gfp0[2 * k - 1];
    ...
    while(true)
    {
        ...
        if(s == 1) {
            ...
            Langrange<gfp0>::evaluate_bases(k, r, eval_base);
            
            for(int i = 0; i < k; i++) {
                final_input += eval_base[i] * input[i][0];
            }
            Langrange<gfp0>::evaluate_bases(2 * k - 1, r, eval_base_2k);
...
void BGIN19Protocol<_T>::gen_vermsg() {
    ...
    for (int i = 0; i < total_zkp_datas; i ++) {
        factory.push(ThreadInfo(VERIFY1, i));
    }
...
void BGIN19Protocol<_T>::_VERIFY(int tid) {
    ...
    gfp0 res = data.vermsg.final_input * data.other_vermsg.final_input;
    gfp0 p_eval_r = data.vermsg.final_result_ss + data.other_vermsg.final_result_ss;
...
bool BGIN19Protocol<_T>::verify() {
    ...
    ws.reset();
    for (int i = 0; i < total_zkp_datas; i ++) {
        factory.push(ThreadInfo(VERIFY, i));
    }
    ws.wait();
        ...
        for (int i = 0; i < k; i++) {
            ...
            for (int j = 0; j < cols; j++) {
                if (temp_pointer >= size) {
                    ...
                    gfp0 x_first = mul_triples[global_batch_size * tid + temp_pointer].x[0].debug();
                    gfp0 y_first = mul_triples[global_batch_size * tid + temp_pointer].y[0].debug();
                    gfp0 x_second = mul_triples[global_batch_size * tid + temp_pointer].x[1].debug();
                    gfp0 y_second = mul_triples[global_batch_size * tid + temp_pointer].y[1].debug();
                    gfp0 rho_first = mul_triples[global_batch_size * tid + temp_pointer].rho[0].debug();
                    gfp0 rho_second = mul_triples[global_batch_size * tid + temp_pointer].rho[1].debug();
                    gfp0 z_second = mul_triples[global_batch_size * tid + temp_pointer].z[1].debug();
