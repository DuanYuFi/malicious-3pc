...
#ifndef PROTOCOLS_Mal3PCArithProtocol_HPP_
#define PROTOCOLS_Mal3PCArithProtocol_HPP_

#include "Mal3PCArithProtocol.h"
...
template <class T>
void Mal3PCArithProtocol<T>::local_init() {
    ...
    E = new VerifyRing[batch_size * ms * zoom_size];                // uint64_t also works

    a_triples = new MulRing[new_batch_size * ms * zoom_size];
    b_triples = new MulRing[new_batch_size * ms * zoom_size];
    c_triples = new MulRing[new_batch_size * ms * zoom_size];
    ...
    for (int i = 0; i < OnlineOptions::singleton.thread_number; i++) {
        std::shared_ptr<std::thread> _thread(new std::thread(&Mal3PCArithProtocol<T>::verify_thread_handler, this));
...
template <class T>
Mal3PCArithProtocol<T>::Mal3PCArithProtocol(Player &P) : ReplicatedBase(P)
{
	local_init();
}
...
template<class T>
void Mal3PCArithProtocol<T>::init_mul() {
...
template <class T>
void Mal3PCArithProtocol<T>::verify_part1(int batch_id) {
...
template <class T>
void Mal3PCArithProtocol<T>::verify_part8(int batch_id) {
...
template <class T>
void Mal3PCArithProtocol<T>::verify_part9(int batch_id) {
...
template <class T>
void Mal3PCArithProtocol<T>::verify_part2(int batch_id) {
...
template <class T>
void Mal3PCArithProtocol<T>::verify_part3(int batch_id) {
...
template <class T>
void Mal3PCArithProtocol<T>::verify_part4(int batch_id) {
...
template <class T>
void Mal3PCArithProtocol<T>::verify_part5(int batch_id) {
...
template<class T>
void Mal3PCArithProtocol<T>::verify_thread_handler() {
 ...
 */
template <class T>
void Mal3PCArithProtocol<T>::verify() {
...
template <class T>
void Mal3PCArithProtocol<T>::prepare_mul(const T& x, const T& y, int n) {
...
template <class T>
void Mal3PCArithProtocol<T>::exchange() {
...
template <class T>
inline T Mal3PCArithProtocol<T>::finalize_mul(int n) {
...
template<class T>
inline void Mal3PCArithProtocol<T>::init_dotprod()
{
	init_mul();
...
template<class T>
inline void Mal3PCArithProtocol<T>::prepare_dotprod(const T& x, const T& y)
{
...
template<class T>
inline void Mal3PCArithProtocol<T>::next_dotprod()
{
...
template<class T>
inline T Mal3PCArithProtocol<T>::finalize_dotprod(int length)
{
	(void) length;
    int n = -1;
    this->dot_counter++;
    ...
    for (int i = 1; i < length; i ++) {
        c_triples[pointer_answer * 2] = 0;
        c_triples[pointer_answer * 2 + 1] = 0;
        pointer_answer ++;
    }
...
template<class T>
T Mal3PCArithProtocol<T>::get_random() {
	T res;
	for (int i = 0; i < 2; i++) {
...
template<class T>
template<class U>
void Mal3PCArithProtocol<T>::trunc_pr(const vector<int>& regs, int size, U& proc,
        false_type)
{
...
template<class T>
template<class U>
void Mal3PCArithProtocol<T>::trunc_pr(const vector<int>& regs, int size,
        U& proc)
{
