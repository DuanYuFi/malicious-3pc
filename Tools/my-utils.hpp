...
public:
    ...
    void append_one_msg(_Field msg) {
        update(msg);
    }

    void append_msges(vector<_Field> msges) {
        for(_Field msg: msges) {
            update(msg);
        }
    }

    _Field get_challenge() {
        _Field r = final();
        return r;
    }
};
...
public:
    ...
    static void append_one_msg(LocalHash<gf2n_long> &hash, uint64_t msg) {
        hash.update(msg);
    }

    static void append_msges(LocalHash<gf2n_long> &hash, vector<uint64_t> msges) {
