...
// using gfp0 = gfp0;
// typedef gf2n_long gfp0;

#define CACHE_SIZE 16

static gfp0 caches[CACHE_SIZE][CACHE_SIZE];

gfp0 customized_inverse(uint64_t i, uint64_t j) {
	if (!caches[i][j].is_zero())	return caches[i][j];
	caches[i][j] = (gfp0(i) - gfp0(j)).invert();
	return caches[i][j];
}

struct ArithDZKProof {
    vector<vector<gfp0>> p_evals_masked;
...
struct ArithVerMsg {
    vector<gfp0> b_ss;
    gfp0 final_input;
    gfp0 final_result_ss;

    ArithVerMsg() {}
    ArithVerMsg(vector<gfp0> b_ss, gfp0 final_input, gfp0 final_result_ss) {
        this->b_ss = b_ss;
        ...
        for(uint64_t j = 0; j < n; j++) {
            ...
            for(uint64_t l = 0; l < n; l++) {
                if (l != j) {
					...
					result[i][j] = result[i][j] * customized_inverse(j, l) * (T(i + n) - T(l));
                }
            }
        }
    }
    ...
    for(uint64_t i = 0; i < n; i++) {
        ...
        for(uint64_t j = 0; j < n; j++) {
            if (j != i) {
				...
				result[i] = result[i] * customized_inverse(i, j) * (r - T(j));
            }
        }
    }
}
